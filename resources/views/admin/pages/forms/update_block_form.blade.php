<form method="POST" action="{{ route('editBlockSave', [$page->id, $block->id]) }}" class="card card-default"
    ...
    <div class="card-header">
        <h3 class="card-title">{{ __('Redaguoti') . ' ' . $block->name }}</h3>
    </div>
    <div class="card-body">
        <div class="row">
            ...
            <div class="col-xl-3 col-md-6 col-6">
                <div class="form-group">
                    ...
                    <select class="custom-select rounded-0 @error('show_experience') is-invalid @enderror"
                        ...
                        <option value="0" @if ($block->show_experience == 0) selected @endif>{{ __('Ne') }}
                        </option>
                        <option value="1" @if ($block->show_experience == 1) selected @endif>{{ __('Taip') }}
                        </option>
                    </select>
